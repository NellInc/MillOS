...
describe('ownershipStore', () => {
    ...
    describe('vesting schedule', () => {
        it('should have 20% vesting at 1 year', () => {
            const state = useOwnershipStore.getState();
            const rule = state.structure.vestingSchedule.find(
                (v) => v.yearsOfService === 1
            );
    ...
    describe('investment proposal voting', () => {
        it('should create investment proposal', () => {
            ...
            expect(newState.capitalDecisions.pendingInvestments[0].title).toBe(
                'New Equipment'
            );
            expect(newState.capitalDecisions.pendingInvestments[0].status).toBe(
                'pending'
            );
        });
        ...
        it('should approve investment with >60% approval', () => {
            ...
            expect(newState.capitalDecisions.approvedInvestments[0].status).toBe(
                'approved'
            );
        });
    ...
    describe('compensation', () => {
        it('should propose compensation', () => {
            ...
            expect(newState.wageSolidarity.workerCompensation['w1'].baseAmount).toBe(
                55000
            );
            expect(
                newState.wageSolidarity.workerCompensation['w1'].peerFeedback
            ).toEqual([]);
        });
        ...
        it('should add peer feedback to compensation', () => {
            ...
            expect(
                newState.wageSolidarity.workerCompensation['w1'].peerFeedback[0]
                    .fromWorkerId
            ).toBe('w2');
        });
    });
