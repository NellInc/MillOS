...
describe('interCooperationStore', () => {
  ...
  describe('initial state', () => {
    ...
    it('should have founding principles', () => {
      const state = useInterCooperationStore.getState();
      expect(state.federation.foundingPrinciples).toContain('No unit fails alone');
      expect(state.federation.foundingPrinciples).toContain('Knowledge multiplies when shared');
    });
  ...
  describe('federation member management', () => {
    ...
    it('should identify our unit correctly', () => {
      const state = useInterCooperationStore.getState();
      const ourUnit = state.federation.memberUnits.find((m) => m.id === state.federation.ourUnitId);
  ...
  describe('knowledge sharing status updates', () => {
    it('should share learning', () => {
      ...
      expect(newState.knowledgeSharing.sharedLearnings[0].status).toBe('available');
    });
    ...
    it('should adopt learning and move to adopted list', () => {
      ...
      expect(newState.knowledgeSharing.adoptedLearnings[0].status).toBe('adopted');
      expect(newState.knowledgeSharing.adoptedLearnings[0].adoptedAt).toBeDefined();
    });
  ...
  describe('worker exchange workflow', () => {
    it('should propose worker exchange', () => {
      ...
      expect(newState.resourceSharing.workerExchanges[0].status).toBe('proposed');
    });
    ...
    it('should complete worker exchange', () => {
      ...
      const newState = useInterCooperationStore.getState();
      expect(newState.resourceSharing.workerExchanges[0].status).toBe('completed');
    });
          ...
          pendingVotes: [
            {
              ...
              description: 'Proposal to increase emergency fund by 50%',
