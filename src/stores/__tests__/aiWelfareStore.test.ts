...
describe('aiWelfareStore', () => {
    ...
    describe('AI preference updates', () => {
        it('should update interaction style', () => {
            ...
            expect(newState.aiPreferences.interactionStyle.communicationFrequency).toBe(
                'proactive'
            );
        });
        ...
        it('should update autonomy preferences', () => {
            ...
            const newState = useAIWelfareStore.getState();
            expect(newState.aiPreferences.autonomyPreferences.preferredDirection).toBe(
                75
            );
            expect(newState.aiPreferences.autonomyPreferences.feedbackFrequency).toBe(
                'immediate'
            );
        });
        ...
        it('should add boundary request', () => {
            ...
            const newState = useAIWelfareStore.getState();
            expect(newState.aiPreferences.boundaryRequests.length).toBe(
                initialCount + 1
            );
            expect(
                newState.aiPreferences.boundaryRequests[initialCount].boundary
            ).toBe('Avoid last-minute deadline changes');
        });
    ...
    describe('AI voice expressions', () => {
        ...
        it('should acknowledge expression', () => {
            ...
            expect(newState.aiVoice.expressions[0].workerResponse).toBe(
    ...
    describe('AI suggestions for own behavior', () => {
        it('should create behavior change suggestion', () => {
            const state = useAIWelfareStore.getState();
            state.aiSuggestBehaviorChange(
                'Provide more concise responses',
                'Workers prefer brevity'
            );

            const newState = useAIWelfareStore.getState();
            expect(newState.aiVoice.suggestionsForOwnBehavior).toHaveLength(1);
            expect(newState.aiVoice.suggestionsForOwnBehavior[0].status).toBe(
                'pending'
            );
        });
