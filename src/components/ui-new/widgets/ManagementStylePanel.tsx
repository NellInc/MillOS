...
export const ManagementStylePanel: React.FC = () => {
  ...
  const { managementGenerosity, setManagementGenerosity, getGrantRate } = useAIConfigStore(
    useShallow((state) => ({
  ...
  const {
    ...
    useShallow((state) => {
      ...
      const highTrust = withPrefs
        ...
        .map((m) => ({
          id: m?.workerId || '',
          name: getWorkerName(m?.workerId || ''),
          trust: m?.preferences?.managementTrust || 0,
        }));
  ...
  const styleConfig =
    managementGenerosity >= 80
      ? { label: 'Generous', color: 'text-green-300', bg: 'bg-green-500/20' }
      : managementGenerosity >= 60
        ? { label: 'Kind', color: 'text-emerald-300', bg: 'bg-emerald-500/20' }
        : managementGenerosity >= 40
          ? { label: 'Balanced', color: 'text-blue-300', bg: 'bg-blue-500/20' }
          : managementGenerosity >= 20
            ? { label: 'Firm', color: 'text-amber-300', bg: 'bg-amber-500/20' }
    ...
    >
      ...
      <div className="p-3 border-b border-slate-700/50">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`ml-auto text-xs font-bold px-2 py-0.5 rounded ${styleConfig.bg} ${styleConfig.color}`}
          >
            {styleConfig.label}
          </span>
        </div>
        <p className="text-[10px] text-slate-400 mt-1">
          Press <kbd className="px-1 py-0.5 bg-slate-700 rounded text-[9px]">B</kbd> to toggle - AI
          evaluates every 10s
        </p>
      </div>
        ...
        <div className="grid grid-cols-4 gap-1" role="group" aria-label="Management style presets">
          {MANAGEMENT_PRESETS.map((preset) => (
            <button
              ...
              className={`px-2 py-1.5 rounded text-[10px] font-medium border transition-all hover:scale-105 ${
      ...
      <div className="p-3 border-b border-slate-700/30">
        <div className="flex items-center gap-2 mb-2">
          ...
          <label htmlFor="management-generosity-slider" className="text-xs font-medium text-white">
            Fine-tune
          </label>
          <span className="ml-auto text-xs font-mono text-cyan-400" aria-live="polite">
      ...
      <div className="p-3 border-b border-slate-700/30">
        <div className="grid grid-cols-4 gap-2">
          ...
          <div className="bg-slate-800/50 rounded p-2 text-center">
            <Heart
              className={`w-3 h-3 mx-auto mb-1 ${averageTrust >= 70 ? 'text-green-400' : averageTrust >= 50 ? 'text-amber-400' : 'text-red-400'}`}
              aria-hidden="true"
            />
          ...
          <div className="bg-slate-800/50 rounded p-2 text-center">
            <TrendingUp
              className={`w-3 h-3 mx-auto mb-1 ${averageInitiative >= 70 ? 'text-cyan-400' : averageInitiative >= 50 ? 'text-amber-400' : 'text-red-400'}`}
              aria-hidden="true"
            />
          ...
          <div className="bg-slate-800/50 rounded p-2 text-center">
            <Activity
              className={`w-3 h-3 mx-auto mb-1 ${pendingCount === 0 ? 'text-green-400' : 'text-amber-400'}`}
              aria-hidden="true"
            />
      ...
      <div className="p-3 border-b border-slate-700/30">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span
            className={`ml-auto text-[10px] font-bold ${rateChange >= 0 ? 'text-green-400' : 'text-red-400'}`}
          >
            {rateChange >= 0 ? '+' : ''}
            {rateChange.toFixed(1)} bags/hr
          </span>
        </div>

        {/* Productivity Bar */}
        <div
          className="relative h-4 bg-slate-800 rounded-full overflow-hidden mb-1"
          role="progressbar"
          aria-valuenow={productivityMultiplier * 100}
          aria-valuemin={0}
          aria-valuemax={120}
          aria-label="Productivity multiplier"
        >
          <motion.div
            initial={{ width: 0 }}
                  ...
                  {highTrustWorkers.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {highTrustWorkers.slice(0, 5).map((w) => (
                        <span
                          key={w.id}
                          className="px-1.5 py-0.5 bg-green-500/20 text-green-300 rounded text-[9px]"
                        >
                      ...
                      {highTrustWorkers.length > 5 && (
                        <span className="text-[9px] text-green-400">
                          +{highTrustWorkers.length - 5} more
                        </span>
                      )}
                    </div>
                  ) : (
                    <span className="text-[9px] text-slate-400 italic">
                      No workers above 70% trust yet
                    </span>
                  )}
                </div>
                  ...
                  {lowTrustWorkers.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {lowTrustWorkers.slice(0, 5).map((w) => (
                        <span
                          key={w.id}
                          className="px-1.5 py-0.5 bg-red-500/20 text-red-300 rounded text-[9px]"
                        >
                          {w.name.split(' ')[0]} ({w.trust.toFixed(0)}%)
                        </span>
                      ))}
                      {lowTrustWorkers.length > 5 && (
                        <span className="text-[9px] text-red-400">
                          +{lowTrustWorkers.length - 5} more
                        </span>
                      )}
                    </div>
                  ) : (
                    <span className="text-[9px] text-slate-400 italic">
                      No workers below 40% trust
                    </span>
                  )}
                </div>
              </div>
            ...
            >
              <div className="mt-3 space-y-2 text-[10px] text-slate-300">
                <p className="leading-relaxed">
                  <strong className="text-cyan-400">Bilateral Alignment</strong> is a philosophy
                  where AI-human partnerships are built on mutual respect. Workers have preferences;
