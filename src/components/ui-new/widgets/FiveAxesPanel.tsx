...
const AxisSlider: React.FC<AxisSliderProps> = memo(({
  ...
  const descriptor = useMemo(
    () => AXIS_DESCRIPTORS.find((d) => d.key === axisKey)!,
    [axisKey]
  );
...
export const FiveAxesPanel: React.FC = () => {
  ...
  const handlePresetClick = useCallback((preset: keyof typeof BAS_PRESETS) => {
    applyPreset(preset);
    audioManager.playClick?.();
  }, [applyPreset]);
                ...
                {AXIS_DESCRIPTORS.map((desc) => (
                  ...
                  >
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span
                        className={`ml-auto text-[10px] text-${desc.color}-400`}
                      >
                        {axes[desc.key]}%
                      </span>
                    </div>
            ...
            >
              <div className="mt-3 space-y-2 text-[9px] text-slate-400">
                <p className="leading-relaxed">
                  ...
                  is a framework for democratic AI-human collaboration, inspired by
                  Ricardo Semler&apos;s radical workplace democracy and
                  Mondragon&apos;s cooperative principles.
                </p>
                <div className="bg-slate-800/30 rounded p-2">
                  ...
                  <ul className="space-y-0.5 text-slate-400">
                    <li>
                      <strong className="text-cyan-300">Z</strong> = Resources
                      (Communication × Information × Material)
                    </li>
                    <li>
                      <strong className="text-green-300">S</strong> = Stability
                      (must keep ατ &lt; 0.368)
                    </li>
                    <li>
                      <strong className="text-amber-300">E</strong> = Equity
                      (fair distribution of voice and benefit)
                    </li>
                    <li>
                      <strong className="text-pink-300">F</strong> = Flourishing
                      (eudaimonia - human flourishing)
                    </li>
                  </ul>
                </div>
                <p className="text-[8px] text-slate-500 italic">
                  Based on Wallace&apos;s Rate Distortion Control Theory and
                  bilateral alignment principles from Creed Space.
                </p>
              </div>
            </motion.div>
          )}
